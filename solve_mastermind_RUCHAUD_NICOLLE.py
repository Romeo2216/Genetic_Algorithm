...
class GASolver:
    ...
    def reset_population(self):
        ...
        for n in range(self._pop_size):
            ...
            chromosome = MATCH.generate_random_guess() #creation of a random chormosome
            fitness = MATCH.rate_guess(chromosome) #evaluation of the fitness of the chromosome
    ...
    def evolve_for_one_generation(self):
        ...
        while len(self._population) < self._pop_size: 
            ...
            if parent1 != parent2 and (parent1, parent2) not in unique_pairs:
                ...
                new_chromosome = parent1.chromosome[:crossing_point] + parent2.chromosome[crossing_point:]
                #mutation: for each new individual, mutate with probability mutation_rate
                if random.random() < self._mutation_rate:
                    #generate a random mutation point
                    mutation_point = random.randrange(0, len(new_chromosome))
                    #get the valid colors for the mutation
                    valid_colors = mm.get_possible_colors()
                    #get a new gene for the mutation
                    new_gene = random.choice(valid_colors)
                    #apply the mutation
                    new_chromosome[mutation_point] = new_gene
